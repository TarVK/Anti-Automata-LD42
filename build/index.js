...
var Board = function () {
    function Board(width, height, gameRules) {
        ...
        // Set up the element
        var pxWidth = width * _Cell2.default.size;
        var pxHeight = height * _Cell2.default.size;
        ...
        for (var x = 0; x < width; x++) {
            ...
            for (var y = 0; y < height; y++) {
                var cell = new _Cell2.default(this, x, y);;
                this.cells[x][y] = cell;
    ...
    _createClass(Board, [{
        key: "setGameRules",
        value: function setGameRules(gameRules) {
            this.tickNumber = -1;
        ...
        key: "getCellState",
        value: function getCellState(xIndex, yIndex) {
            var cell = this.getCell(xIndex, yIndex);
            if (cell) return cell.getState(this.tickNumber - 1);
            return this.gameRules.edgeState;
        }
    }, {
        key: "getCell",
        value: function getCell(xIndex, yIndex) {
...
var Cell = function () {
    function Cell(board, xIndex, yIndex) {
        var _this = this;
        ...
        var pxX = (this.x + 0.5) * Cell.size;
        var pxY = (this.y + 0.5) * Cell.size;
        this.element = (0, _jquery2.default)("\n            <div class=board-cell style=left:" + pxX + "px;top:" + pxY + "px;>\n                <div class=board-cell-inner>\n\n                </div>\n            </div>\n        ");
        this.element.mouseup(function (event) {
            _this.board.tickNumber++;
            _this.board.gameRules.cellClick.call(_this, _this, _this.getState(), event);
            _this.board.tickNumber--;
            event.preventDefault();
        });
    ...
    _createClass(Cell, [{
        ...
        value: function tick(_tick) {
            ...
            this.tickNumber = _tick;
            var state = this.board.gameRules.cellTick.call(this, this, currentState);
            this.setState(state);
        }
    }, {
        key: "getNeighbor",
        value: function getNeighbor(xOffset, yOffset) {
            var xIndex = this.x + xOffset;
            var yIndex = this.y + yOffset;
            return this.board.getCellState(xIndex, yIndex);
        }
    }, {
        key: "getNeighbors",
        value: function getNeighbors(pattern) {
            var _this2 = this;

            var cells = [];
            pattern.getCells().forEach(function (offset) {
                cells.push(_this2.getNeighbor(offset.x, offset.y) * offset.multiplier);
            });
            return cells;
        }
...
function gradients(number, c1, c2) {
    ...
    for (var i = 0; i < number; i++) {
        var per = i / (number - 1);
        var c = [Math.floor(Math.sqrt(c1[0] * c1[0] * (1 - per) + c2[0] * c2[0] * per)) + 256, // +256 for leading 1
        Math.floor(Math.sqrt(c1[1] * c1[1] * (1 - per) + c2[1] * c2[1] * per)) + 256, Math.floor(Math.sqrt(c1[2] * c1[2] * (1 - per) + c2[2] * c2[2] * per)) + 256];
...
var GameRules = function GameRules(data) {
    var _this = this;
    ...
    this.cellClick = d(data.cellClick, function (cell) {
        var state = cell.getState();
        var nextState = _this.clickTransitions[state];
        if (nextState !== undefined) cell.setState(nextState);
    });
    this.cellTick = data.cellTick;
    this.tick = d(data.tick, function () {});

    this.style = (0, _jquery2.default)("<style>" + (data.style || "") + this.colors.map(function (color, index) {
        var borderColor = colorPer("#777777", color, 0.5);
...
var Pattern = function () {
    function Pattern(rawPattern) {
        _classCallCheck(this, Pattern);

        var pattern = rawPattern.replace(/(?=\s)[^\n]|^\n\s*|\n?\s*\n?$/gm, "");

        var lines = pattern.split("\n");
        var width = lines[0].length;
        var height = lines.length;
        var xStartOffset = Math.floor(-(width - 1) / 2);
        var yStartOffset = Math.floor(-(height - 1) / 2);
        ...
        for (var x = 0; x < width; x++) {
            for (var y = 0; y < height; y++) {
                var char = lines[y][x];
                if (!char.match(/\./)) {
                    var xIndex = x + xStartOffset;
                    var yIndex = y + yStartOffset;
...
Array.prototype.sum = function (func) {
    return this.reduce(function (prev, item) {
        return prev + map(item, func);
    }, 0);
};
Array.prototype.min = function (func) {
    var min = Infinity;
    this.forEach(function (item) {
        if (item < min) min = map(item, func);
    });
    return min;
};
...
Array.prototype.contains = function (searchItem, func) {
    ...
    if (typeof searchItem == "function") {
        ...
        this.forEach(function (item) {
            if (map(item, func) == searchItem) count++;
        });
    }
    return count;
};
...
var _keyboard = __webpack_require__(/*! ./keyboard */ "./src/keyboard.js");

var _keyboard2 = _interopRequireDefault(_keyboard);

__webpack_require__(/*! ./arrayAugment */ "./src/arrayAugment.js");
...
var keys = global.keys = {
    w: (0, _keyboard2.default)(87, 38),
    a: (0, _keyboard2.default)(65, 37),
    s: (0, _keyboard2.default)(83, 40),
    d: (0, _keyboard2.default)(68, 39),
    space: (0, _keyboard2.default)(32)
}; // Libraries
...
global.stop = function () {
    ...
    setTimeout(function () {
        (0, _jquery2.default)(".app-dialogue").addClass("shown");
        (0, _jquery2.default)(".app-menu").addClass("restart");
    }, 1000);
    pauseMusic(true);
};

/* Start controls */
keys.space.press = function () {
    if ((0, _jquery2.default)(".app-dialogue").is(".shown")) {
        if (Date.now() - deathTime > 1000) (0, _jquery2.default)(".app-play-button").addClass("active");
    }
};
keys.space.release = function () {
    if ((0, _jquery2.default)(".app-dialogue").is(".shown")) (0, _jquery2.default)(".app-play-button.active").removeClass("active").click();
};
(0, _jquery2.default)(".app-play-button").click(function () {
    if ((0, _jquery2.default)(".app-dialogue").is(".shown")) {
        start();
        (0, _jquery2.default)(".app-dialogue").removeClass("shown");
    }
});

/* Audio controls */
// Sound fx
var song = (0, _jquery2.default)(".audio-song")[0];
var fxMultiplier = 0.5;
global.sounds = {
    blop: { sound: (0, _jquery2.default)(".audio-blop")[0], multiplier: 0.2 },
    hit: { sound: (0, _jquery2.default)(".audio-hit")[0], multiplier: 1 },
    hitSpread: { sound: (0, _jquery2.default)(".audio-hit-spread")[0], multiplier: 1 },
    pew: { sound: (0, _jquery2.default)(".audio-pew")[0], multiplier: 1 },
    gameOver: { sound: (0, _jquery2.default)(".audio-game-over")[0], multiplier: 1 }
};
global.playSound = function (soundData) {
    if (soundFxEnabled) {
        var sound = soundData.sound.cloneNode(true);
...
function setMusicEnabled(enabled) {
    ...
    if (enabled) (0, _jquery2.default)(".app-music").removeClass("disabled");else (0, _jquery2.default)(".app-music").addClass("disabled");
    if (enabled) {
        song.play();
    } else {
        song.pause();
...
function setSoundFxEnabled(enabled) {
    ...
    if (enabled) (0, _jquery2.default)(".app-sound-fx").removeClass("disabled");else (0, _jquery2.default)(".app-sound-fx").addClass("disabled");
}
(0, _jquery2.default)(".app-sound-fx").mousedown(function (e) {
    if (soundFxEnabled) setSoundFxEnabled(false);else setSoundFxEnabled(true);
    e.preventDefault();
});
setMusicEnabled(musicEnabled);
...
function setVolume(per) {
    ...
    volume = per;
    volEl.children().first().width((per * 100 + "").substr(0, 4) + "%");
    song.volume = per;
}
var volEl = (0, _jquery2.default)(".app-volume").mousedown(function (e) {
    var offset = volEl.offset();
...
/* scoring */
var multiplier = 1;
var highscore = localStorage && localStorage.getItem("highscore") || 0;
var score = 0;
...
function setScore(val) {
    score = val;
    (0, _jquery2.default)(".app-score-value").text(Math.floor(score));
    (0, _jquery2.default)(".app-final-score-value").text(Math.floor(score));
    if (score > highscore) {
        setHighScore(score);
    }
}
function setHighScore(score) {
    highscore = score;
    (0, _jquery2.default)(".app-highscore-value").text(Math.floor(highscore));
    if (localStorage) localStorage.setItem("highscore", Math.floor(highscore));
}
setHighScore(highscore);
global.setMultiplier = function (val) {
    multiplier = val;
    (0, _jquery2.default)(".app-score-multiplier-value").text(Math.floor(multiplier) + "X");
};
...
function handleKey(keyCode) {
    if (arguments.length > 1) {
        ...
        _key.isDown = false;
        _key.isUp = true;
        _key.press = undefined;
        _key.release = undefined;
        //The `downHandler`
        _key.downHandler = function (event) {
            if (event.keyCode === _key.code) {
                if (_key.isUp && _key.press) _key.press();
                _key.isDown = true;
                _key.isUp = false;
                event.preventDefault();
            }
        ...
        //The `upHandler`
        _key.upHandler = function (event) {
            if (event.keyCode === _key.code) {
                if (_key.isDown && _key.release) _key.release();
                _key.isDown = false;
                _key.isUp = true;
                event.preventDefault();
            }
        ...
        window.addEventListener("keydown", _key.downHandler.bind(_key), false);
        window.addEventListener("keyup", _key.upHandler.bind(_key), false);
        return _key;
    }
}
...
var spreadChance = 0.015;
// Set up the game
var spreadPatternOuter = new _Pattern2.default('\n    ..1..\n    .a.a.\n    1...1\n    .a.a.\n    ..1..\n');
...
exports.default = new _GameRules2.default({
    cellTick: function cellTick(cell, current) {
        ...
        if ([keys.w.isDown, keys.a.isDown, keys.s.isDown, keys.d.isDown].contains(true) == 1) {
            if (current == 1) {
                move: if (keys.w.isDown || keys.a.isDown || keys.s.isDown || keys.d.isDown) {
                    ...
                    if (Math.random() < spreadChance * 1.5 && cells.contains(function (state) {
        ...
        if (current == 1) {
            cells = cell.getNeighbors(spreadPattern);
            if (Math.random() < spreadChance && cells.contains(function (state) {
                return state >= 5 && state < 10;
            })) return 5;
            return current;
        } else if (current == 2) {
        ...
        if (current == 0) {
            ...
            cells = cell.getNeighbors(spreadPattern);
            if (cells.contains(function (state) {
                return state == 3;
            })) {
    ...
    tick: function tick(board) {
        ...
        var totalPoints = board.tickPoints.reduce(function (a, b) {
            return a + b;
        }, 0);
        var multiplier = Math.min(8, Math.floor(1 + totalPoints / 25));
        global.setMultiplier(multiplier);

        var recentPoints = board.tickPoints.slice(0, 10).reduce(function (a, b) {
            return a + b;
        }, 0);
        if (recentPoints > 10) {
            ...
            shakeTimeout = setTimeout(function () {
                return board.element.removeClass("shake shakeSmall shakeBig");
            }, 100);
        }
